...
MODEL_NAME = "gpt-4o-mini"  # <--- 全局变量：方便在此修改模型名称，如 "gpt-4o"
...
class DocScraper:
    ...
    def analyze_img_with_ai(self, img_url):
        """使用全局定义的 MODEL_NAME 解析图片"""
        b64_data = self.get_image_base64(img_url)
        if not b64_data: return "[无法读取图片内容]"
        
        try:
            res = client.chat.completions.create(
                model=MODEL_NAME,  # <--- 调用全局变量
                messages=[{"role": "user", "content": [
                    {"type": "text", "text": "请简要描述这张图片的内容，并提取其中的文字信息。"},
    ...
    def html_to_md(self, element):
        ...
        match tag:
            ...
            case 'img':
                src = element.get('src', '')
                desc = self.analyze_img_with_ai(src) if src else "无链接"
                return f"\n\n![img]({src})\n> **AI 图片解析**: {desc}\n\n"
